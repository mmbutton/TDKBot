...
async def help(interaction):
    ...
    helpStr += (
        command_names.APP_GROWTH_TIER_LIST
        + ": Tier list of heroes based on how fast they grow when investing quality. Use Military, Provisions, etc. to see the list for that hero\n"
    )
    helpStr += (
        command_names.APP_ATTRIBUTE_TIER_LIST
        + ": Tier list for heroes based on the maximum attribute in each stat. Includes Military, provisions etc. but also includes KP and Power.\n"
    )
    await interaction.response.send_message(helpStr)


@client.tree.command(
    description="Pulls up a formula sheet with a bunch of useful formulas such as KP, power etc."
)
async def formulas(interaction):
    await interaction.response.send_message(
        file=discord.File(Path(__file__).parent / "../resources/formulas.png")
    )


@client.tree.command(
    description="Pulls up a screenshot showing Zodiacs, their maidens and their paragons."
)
async def zodiacs(interaction):
    await interaction.response.send_message(
        file=discord.File(Path(__file__).parent / "../resources/zodiacs.png")
    )


@client.tree.command(
    description="Pulls up a screenshot of all current castle skins and their effects."
)
async def castle_skins(interaction):
    await interaction.response.send_message(
        file=discord.File(Path(__file__).parent / "../resources/castle_skins.png")
    )


@client.tree.command(
    description="Posts an image of the event schedule for challenges and cross server events."
)
async def event_schedule(interaction):
    await interaction.response.send_message(
        file=discord.File(Path(__file__).parent / "../resources/event_schedule.png")
    )


@client.tree.command(
    description="Pulls up Haka's inforgraphic showing manuscript batch efficiency"
)
async def manuscript_efficiency(interaction):
    await interaction.response.send_message(
        file=discord.File(Path(__file__).parent / "../resources/manu_efficiency.png")
    )


@client.tree.command(
    description="Get a hero inforgraphic created by Deadpool♾825's or Milo's for a selected hero"
)
...
async def hero_infographic(interaction, hero_name: str):
    ...
    if filename is not None:
        await interaction.response.send_message(
            file=discord.File(Path(__file__).parent / filename)
        )
    else:
        diffs = hero_collection.hero_name_diff(hero_name)
        await interaction.response.send_message(
            "Hero " + hero_name + " not found. Close hero names: " + str(diffs)
        )


@client.tree.command(description="Get hero statistics")
@app_commands.describe(
    hero_name="Name of the hero to get", detailed="Use detailed statistics"
)
async def hero(interaction, hero_name: str, detailed: bool = True):
    ...
    if entry is not None:
        ranks = [
            command.ordinal(
                hero_collection.hero_rank(hero, hero_collection.TierList.KINGDOM_POWER)
            ),
            command.ordinal(
                hero_collection.hero_rank(hero, hero_collection.TierList.MILITARY_POWER)
            ),
            command.ordinal(
                hero_collection.hero_rank(hero, hero_collection.TierList.MILITARY)
            ),
            command.ordinal(
                hero_collection.hero_rank(hero, hero_collection.TierList.FORTUNE)
            ),
            command.ordinal(
                hero_collection.hero_rank(hero, hero_collection.TierList.PROVISIONS)
            ),
            command.ordinal(
                hero_collection.hero_rank(hero, hero_collection.TierList.INSPIRATION)
            ),
        ]
        if detailed:
            ...
            response_str = (
                response_str
                + "```Max Attributes (lvl 400)\nMax Power {0} | Max KP {1} | Max Military {2} | Max Fortune {3} | Max Provisions {4} | Max Inspiration {5})```".format(
                    command.format_big_number(entry.max_power),
                    command.format_big_number(entry.max_kp),
                    command.format_big_number(entry.max_military),
                    command.format_big_number(entry.max_fortune),
                    command.format_big_number(entry.max_provisions),
                    command.format_big_number(entry.max_inspiration),
                )
            )
            response_str = (
                response_str
                + "```Base Quality\n Military {0} | Fortune {1} | Provisions {2} | Inspiration {3}```".format(
                    entry.military_quality,
                    entry.fortune_quality,
                    entry.provisions_quality,
                    entry.inspiration_quality,
                )
            )
            response_str = (
                response_str
                + "```Quality Efficiency %\n Military {0}% | Fortune {1}% | Provisions {2}% | Inspiration {3}%```".format(
                    entry.military_growth,
                    entry.fortune_growth,
                    entry.provisions_growth,
                    entry.inspiration_growth,
                )
            )
            response_str = (
                response_str
                + "```Paragon % (Tome efficiency)\n Military {0}% | Fortune {1}% | Provisions {2}% | Inspiration {3}%```".format(
                    int(entry.military_paragon * 100),
                    int(entry.fortune_paragon * 100),
                    int(entry.provisions_paragon * 100),
                    int(entry.inspiration_paragon * 100),
                )
            )
            response_str = (
                response_str
                + "```\nRank (KP | Power)\n Max KP {0} | Power {1}```".format(
                    ranks[0], ranks[1]
                )
            )
            response_str = (
            ...
            response_str = (
                response_str
                + "```Max KP Rating: {0} | Max Power Rating: {1} | Military Growth Rank: {2} | Fortune Growth Rank: {3} | Provisions Growth Rank: {4} |".format(
                    ranks[0], ranks[1], ranks[2], ranks[3], ranks[4]
                )
            )
            response_str = (
                response_str
                + "Inspiration Growth Rank: {0} | Difficulty {1}```".format(
                    ranks[5], entry.difficulty
                )
            )
            await interaction.response.send_message(response_str)
    else:
        ...
        await interaction.response.send_message(
            "Hero " + hero + " not found. Close hero names: " + str(diffs)
        )


@client.tree.command(
    description="Generate a tier list based on max attributes of heroes"
)
@app_commands.describe(
    type="Type of tier list to generate", size="Size of the tier list"
)
async def attribute_tier_list(
    interaction,
    type: hero_collection.TierList,
    size: app_commands.Range[int, 0, 200] = 10,
):
    ...
    for hero in tier_list:
        ...
        tier_list_str += "{0}. {1} ({2})\n".format(
            rank, hero.hero_name, command.format_big_number(attribute)
        )
        rank += 1
...
async def growth_tier_list(
    interaction,
    type: hero_collection.GrowthTierList,
    size: app_commands.Range[int, 0, 200] = 10,
):
    ...
    for hero in tier_list:
        ...
        tier_list_str += "{0}. {1} ({2}%, {3})\n".format(
            rank, hero.hero_name, round(growth), command.format_big_number(attribute)
        )
        rank += 1
...
def jotun_notifier():
    asyncio.run_coroutine_threadsafe(
        send_message_to_channel(
            ...
            "<@&" + os.getenv("BL_ALERTS") + "> Jotun time",
        ),
        client.loop,
    )
    ...
    asyncio.run_coroutine_threadsafe(
        send_message_to_channel(
            int(os.getenv("COLLECTIVE")),
            "<@&" + os.getenv("COLLECTIVE_ALERTS") + "> Jotun time",
            True,
        ),
        client.loop,
    )
    ...
    asyncio.run_coroutine_threadsafe(
        send_message_to_channel(
            int(os.getenv("MACKENZIE")),
            "@everyone\n Jotun himself has Joined the fight!",
        ),
        client.loop,
    )

    asyncio.run_coroutine_threadsafe(
        send_message_to_channel(int(os.getenv("S941")), "@everyone Jotun time"),
        client.loop,
    )
...
def jotun_minions_notifier():
    asyncio.run_coroutine_threadsafe(
        send_message_to_channel(
            int(os.getenv("GENERAL_CHAT")),
            "<@&" + os.getenv("BL_ALERTS") + "> Jotun's minions time",
        ),
        client.loop,
    )
    ...
    asyncio.run_coroutine_threadsafe(
        send_message_to_channel(
            int(os.getenv("COLLECTIVE")),
            "<@&" + os.getenv("COLLECTIVE_ALERTS") + "> Jotun's minions time",
            True,
        ),
        client.loop,
    )
    ...
    asyncio.run_coroutine_threadsafe(
        send_message_to_channel(
            int(os.getenv("MACKENZIE")),
            "@everyone\n Jotun has sent a surprise attack!!! Fight him back everyone!!!",
        ),
        client.loop,
    )
    ...
    asyncio.run_coroutine_threadsafe(
        send_message_to_channel(
            int(os.getenv("S941")), "@everyone Jotun's minions time"
        ),
        client.loop,
    )
...
def server_reset_notifier():
    asyncio.run_coroutine_threadsafe(
        send_message_to_channel(
            int(os.getenv("COLLECTIVE")),
            "<@&"
            + os.getenv("COLLECTIVE_ALERTS")
            + "> Daily server rest will be in 15 minutes",
            True,
        ),
        client.loop,
    )
    ...
    asyncio.run_coroutine_threadsafe(
        send_message_to_channel(
            int(os.getenv("MACKENZIE")),
            "@everyone Server will reset in 15 minutes. Be ready to collect your daily tithes and keep your maidens company!",
        ),
        client.loop,
    )
    ...
    asyncio.run_coroutine_threadsafe(
        send_message_to_channel(
            int(os.getenv("S941")), "@everyone Daily server rest will be in 15 minutes"
        ),
        client.loop,
    )
...
def boss_free_for_all_notifier():
    asyncio.run_coroutine_threadsafe(
        send_message_to_channel(
            ...
            "<@&"
            + os.getenv("BL_ALERTS")
            + "> Bosses will be a free for all in 15 minutes. The invincible boss will be opened in 30min",
        ),
        client.loop,
    )
...
def cross_server_notifier():
    asyncio.run_coroutine_threadsafe(
        send_message_to_channel(
            ...
            "<@&"
            + os.getenv("BL_ALERTS")
            + "> New cross server fight is open. Please deploy a hero in the alliance hall.",
        ),
        client.loop,
    )
    ...
    asyncio.run_coroutine_threadsafe(
        send_message_to_channel(
            int(os.getenv("MACKENZIE")),
            "@everyone A new cross server fight begins!!! Send your hero to battle in the alliance hall!",
        ),
        client.loop,
    )
    ...
    asyncio.run_coroutine_threadsafe(
        send_message_to_channel(
            int(os.getenv("S941")),
            "@everyone New cross server fight is open. Please deploy a hero in the alliance hall.",
        ),
        client.loop,
    )
