...
async def parse_tier_list_args(message, prog, command):
    try:
        ...
        await message.channel.send(
            'Unknown arguments detected. Only "-l" flag for low_vip or "-n" for new players are accepted for arguments'
        )
        return -1, False
...
async def _send_long_string_msg(discordMessage, messageString):
    if (
        discordMessage.channel.type != discord.ChannelType.private
        and len(messageString) > 1000
    ):
        await discordMessage.channel.send(_LONG_BOT_LIST_MSG)
        return
    for chunk in [
        messageString[i : i + 1999] for i in range(0, len(messageString), 1999)
    ]:
...
async def hero(message, command_str):
    command_str = (
        command_str[(len(command_names.HERO) + 1) :].replace("‘", "'").replace("’", "'")
    )
    if command_str.startswith("-i"):
        ...
        if filename is not None:
            ...
            await message.channel.send(
                "Hero " + command_str + " not found. Close hero names: " + str(diffs)
            )
        return
        ...
        if detailed:
            ...
            response_str = (
                ...
                + "```Max Attributes (lvl 400)\nMax Power {0} | Max KP {1} | Max Military {2} | Max Fortune {3} | Max Provisions {4} | Max Inspiration {5}```".format(
                    format_big_number(entry.max_power),
                    format_big_number(entry.max_kp),
                    format_big_number(entry.max_military),
                    format_big_number(entry.max_fortune),
                    format_big_number(entry.max_provisions),
                    format_big_number(entry.max_inspiration),
                )
            )
            response_str = (
                response_str
...
async def power_tier_list(message, command_str):
    difficulty, attributes, listLength = await parse_tier_list_args(
        message, command_names.POWER_TIER_LIST, command_str
    )
    if difficulty < 0:
        return
...
async def kp_tier_list(message, command_str):
    difficulty, attributes, listLength = await parse_tier_list_args(
        message, command_names.KP_TIER_LIST, command_str
    )
    if difficulty < 0:
        return
...
async def military_tier_list(message, command_str):
    difficulty, attributes, listLength = await parse_tier_list_args(
        message, command_names.MILITARY_TIER_LIST, command_str
    )
    if difficulty < 0:
        return
    if attributes:
    ...
    tier_list_str = (
        "**Military Tier List (" + tier_list_type + ")** " + difficulty_type + "\n"
    )
    ...
    for hero in tier_list:
        if attributes:
            ...
            tier_list_str += (
                ...
                + hero.hero_name
                + " ("
                + str(round(hero.military_growth))
                + "%, "
                + format_big_number(hero.max_military)
                + ")\n"
            )
...
async def fortune_tier_list(message, command_str):
    difficulty, attributes, listLength = await parse_tier_list_args(
        message, command_names.FORTUNE_TIER_LIST, command_str
    )
    if difficulty < 0:
        return
    if attributes:
    ...
    tier_list_type = "Max Attributes" if attributes else "Quality Efficiency"
    tier_list_str = (
        "**Fortune Tier List (" + tier_list_type + ")** " + difficulty_type + "\n"
    )
...
async def provisions_tier_list(message, command_str):
    difficulty, attributes, listLength = await parse_tier_list_args(
        message, command_names.PROVISIONS_TIER_LIST, command_str
    )
    if difficulty < 0:
        return
    if attributes:
    ...
    tier_list_type = "Max Attributes" if attributes else "Quality Efficiency"
    tier_list_str = (
        "**Provisions Tier List (" + tier_list_type + ")** " + difficulty_type + "\n"
    )
    ...
    for hero in tier_list:
        if attributes:
            ...
            tier_list_str += (
                ...
                + hero.hero_name
                + " ("
                + str(round(hero.provisions_growth))
                + "%, "
                + format_big_number(hero.max_provisions)
                + ")\n"
            )
...
async def inspiration_tier_list(message, command_str):
    difficulty, attributes, listLength = await parse_tier_list_args(
        message, command_names.INSPIRATION_TIER_LIST, command_str
    )
    if difficulty < 0:
        return
    if attributes:
    ...
    tier_list_type = "Max Attributes" if attributes else "Quality Efficiency"
    tier_list_str = (
        "**Inspiration Tier List (" + tier_list_type + ")** " + difficulty_type + "\n"
    )
    ...
    for hero in tier_list:
        if attributes:
            ...
            tier_list_str += (
                ...
                + hero.hero_name
                + " ("
                + str(round(hero.inspiration_growth))
                + "%, "
                + format_big_number(hero.max_inspiration)
                + ")\n"
            )
...
async def help(message):
    ...
    helpStr += (
        ...
        + ": Display Deadpool♾825's or Milo's infographic for a selected hero\n"
    )
    helpStr += (
